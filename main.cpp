...
int main() {
    ...
    cout << "-------------------------------------------------\n";
    ifstream readReport("final_report.txt");
    if (readReport.is_open()) {
        string reportLine;
        while (getline(readReport, reportLine)) {
            cout << reportLine << "\n";
        }
        readReport.close();
    }
    cout << "-------------------------------------------------\n";
    cout << "Success! All files generated. Check your folder for details.\n\n";
