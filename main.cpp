...
/*
    ...
    For each sequence in BST1:
      ...
      - If it does NOT exist in BST2 --> it is unique to Lab A, save to unique1File
*/
void collectCommonAndUnique(Node* node, BST& bst2,
                            ofstream& commonFile, ofstream& unique1File,
                            int& commonCount) {
    if (node == nullptr) return;   // base case: reached an empty branch, stop

    // visit left side first (in-order traversal)
    collectCommonAndUnique(node->leftChild, bst2, commonFile, unique1File, commonCount);
    ...
    collectCommonAndUnique(node->rightChild, bst2, commonFile, unique1File, commonCount);
}
...
/*
    ...
    For each sequence in BST2:
      - If it does NOT exist in BST1 --> it is unique to Lab B, save to unique2File
      - If it already exists in BST1 --> skip it (already counted as common above)
*/
void collectUniqueInBST2(Node* node, BST& bst1, ofstream& unique2File) {
    if (node == nullptr) return;   // base case: empty branch, stop
...
int main() {
    ...
    if (!fileA.is_open()) {
        cout << "[ERROR] Cannot open lab_a_data.txt -- make sure the file exists.\n";
        return 1;
    }
    int linesA = 0;
    ...
    if (!fileB.is_open()) {
        cout << "[ERROR] Cannot open lab_b_data.txt -- make sure the file exists.\n";
        return 1;
    }
    int linesB = 0;
    ...
    if (!commonFile.is_open() || !unique1File.is_open() || !unique2File.is_open()) {
    ...
    collectCommonAndUnique(bst1.getRoot(), bst2, commonFile, unique1File, commonCount);
    ...
    // ---------------------------------------------------------------
    int    unionSize  = totalA + totalB - commonCount;  // total unique sequences across both labs
    ...
    string level;
    if      (similarity >= 70.0) level = "High";
    else if (similarity >= 40.0) level = "Medium";
    else                          level = "Low";
    ...
    report
        ...
        << "------------------------------------------------------------\n"
        << "  - " << commonCount << " gene sequence(s) are shared between Lab A and Lab B.\n"
        << "  - These shared sequences may indicate common genetic markers\n"
        << "    found in patients across both the Northern and Southern regions.\n"
        << "  - See common_patterns.txt for the full list with lengths.\n\n"

        << "------------------------------------------------------------\n"
        << "  SECTION 4 : DIFFERENCES BETWEEN DATASETS\n"
        << "------------------------------------------------------------\n"
        << "  - Lab A has " << exclusiveA << " unique sequence(s) not present in Lab B.\n"
        << "    (see nc_element_bst1.txt)\n"
        << "  - Lab B has " << exclusiveB << " unique sequence(s) not present in Lab A.\n"
    ...
    report
        << "  - All 4 output files were generated successfully.\n"
    ...
    cout << "\n[BST1 - Lab A]  Lines read from file: " << linesA << "\n";
    ...
    cout << "\n[BST2 - Lab B]  Lines read from file: " << linesB << "\n";
